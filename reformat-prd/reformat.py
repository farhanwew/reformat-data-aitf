...
def map_label(raw_label: str) -> str:
    if not raw_label or _is_na(raw_label):
...
def _validate_result_payload(payload: dict[str, Any]) -> dict[str, str]:
    ...
    return {key: payload[key] if isinstance(payload[key], str) else str(payload[key]) for key in REQUIRED_RESPONSE_KEYS}
...
def _debug_parse_failure(response: Any, attempt: int, row_index: int | None) -> None:
    ...
    print(
        ...
        f" refusal={bool(refusal)} text_len={len(raw_text)} snippet={snippet!r}"
    )
...
def _append_parse_failure_log(
    ...
    payload = {
        ...
        "finish_reason": finish_reason, "refusal": bool(refusal),
...
def _build_result_row(
    ...
    link_value = _single_line(row_data.get("url", "")) or _single_line(row_data.get("link", ""))
    return {
        "link": link_value,
        "source": _single_line(row_data.get("platform", "")),
        "ringkasan": _single_line(parsed.ringkasan),
        "klaim": _single_line(parsed.klaim),
        "fakta": _single_line(parsed.fakta or "tidak ada"),
        "label": _single_line(map_label(raw_label)),
        "analisis": _single_line(parsed.analisis or clean_analisis(raw_analisis)),
    }
...
def _process_one_row(
    ...
    caption = _build_llm_input(row_data)
    raw_label = row_data.get("kategori", "")
    parts = []
    for col in ("analisis_pelanggaran", "analisis_dampak"):
        val = str(row_data.get(col, "")).strip()
        if val and val.lower() != "nan":
            parts.append(val)
    raw_analisis = " ".join(parts)
    ...
    if not analisis_raw or analisis_raw.lower() == "nan":
        ...
        result = _build_result_row(row_data, parsed, raw_label, raw_analisis)
        result["_input"] = ""
...
def reformat_csv(
    ...
    required = {"platform", "analisis_pelanggaran", "kategori"}
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    outer_parts = [v for col in ("analisis_pelanggaran", "analisis_dampak")
                                   if (v := str(row_dict.get(col, "")).strip()) and v.lower() != "nan"]
                    ...
                    result = _build_result_row(row_dict, fallback, row_dict.get("kategori", ""), " ".join(outer_parts))
                    result["_input"] = ""
                    result["_usage"] = _zero_usage()
...
def retry_failed_run(
    ...
    # Rebuild output files dari merged_df yang sudah diupdate
    all_rows = [
    ...
    # Append retry info ke run_info.txt
    info_path = os.path.join(run_dir, "run_info.txt")
    duration = (end_time - start_time).total_seconds()
    ...
    fixed = total_failed - still_failed
    print(f"\nSelesai. {fixed}/{total_failed} baris berhasil diperbaiki.", end="")
    if still_failed:
